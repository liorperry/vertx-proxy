...
public class CouchbaseHealthReporter implements HealthReporter {

    private JedisPool pool;
    private int expirationTime = 5;

    public CouchbaseHealthReporter(JedisPool pool, int expirationTime) {
        this.pool = pool;
        this.expirationTime = expirationTime;
    }

    public HealthCheck.Result report(HealthCheck.Result result,String domain,ServiceDescriptor descriptor) {
        String key = HealthReporter.buildKey(domain, descriptor);
